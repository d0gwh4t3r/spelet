...
class Bird
  ...
  def update
    @velocity += GRAVITY # fågeln faller nedåt
    @base_y += @velocity
    @img.y = @base_y - @radius
    if @small_timer > 0
      @small_timer -= 1
      grow_normal if @small_timer == 0 # powerup tar slut
    end
  end

  def jump; @velocity = JUMP end
  def small?; @small_timer > 0 end

  def shrink
    @img.width = @img.height = 20 # krymper fågeln
    @radius = 7
    @small_timer = 180 # varar 3 sekunder
  end
  ...
  def x; @img.x + @img.width / 2 end
  def y; @img.y + @img.height / 2 end
end
...
class Pipe
  ...
  def initialize
    top_h = rand(80..300) # slumpmässig höjd
  ...
  def update
    @x -= PIPE_SPEED # rör sig åt vänster
    @top.x = @bot.x = @x
  end

  def off_screen?; @x < -100 end
  def mark_passed; @passed = true end
...
class BouncingSpike
  ...
  def update
    @x -= PIPE_SPEED
    @y += @speed * @dir
    @dir = -@dir if @y < 80 || @y > 460 # byter riktning vid kanten
    @img.x = @x - 30
    @img.y = @y - 30
  end
...
class ScoreManager
  ...
  def add_point
    ...
    if @score > @highscore # nytt rekord
      @highscore = @score
...
class Game
  ...
  def initialize
    ...
    @spikes = []
    @powerups = []
    @spawn_timer = @spike_timer = @powerup_timer = 0
    @game_over = false
  ...
  def update
    ...
    # spawnar objekt med jämna mellanrum
    @spawn_timer += 1
    if @spawn_timer > 90
      @pipes << Pipe.new
      @spawn_timer = 0
    ...
    # uppdaterar pelarna och ger poäng när fågeln passerar
    @pipes.each do |pipe|
      pipe.update
      if !pipe.passed && pipe.x + 105 < @bird.x
        pipe.mark_passed
        @score_mgr.add_point
      end
      @game_over = true if pipe.collision?(@bird)
    end
    @pipes.reject! { |p| p.off_screen? && p.remove || false }

    @spikes.each { |s| s.update; @game_over = true if s.collision?(@bird) }
    @spikes.reject! { |s| s.off_screen? && s.remove || false }

    @powerups.each do |p|
      p.update
      if p.collect?(@bird)
        p.remove
        @bird.shrink # krymper fågeln
      end
    end
    @powerups.reject! { |p| !p.active || p.off_screen? && p.remove || false }

    # fågeln dör om den nuddar taket eller golvet
    @game_over = true if @bird.y - @bird.radius < 40 || @bird.y + @bird.radius > 540

    # visar game over bilden när spelet är över
  ...
  def jump_or_restart
    if @state == :start
      [@start_screen, @start_button, @start_label].each(&:remove)
      @state = :playing
      @ground_bot = Image.new("ground.png", x: 0, y: 540, width: 940, height: 40, z: 1)
      @ground_top = Image.new("ground.png", x: 0, y: 0,   width: 940, height: 40, z: 1)
    elsif @game_over
      [@pipes, @spikes, @powerups].each { |a| a.each(&:remove); a.clear }
      @small_text&.remove
      @small_text = nil
      @gameover_img&.remove # tar bort game over bilden
      @gameover_img = nil
      @bird.reset
      @score_mgr.reset
      @spawn_timer = @spike_timer = @powerup_timer = 0
...
on(:mouse_down) { |e| game.jump_or_restart if game.state == :start && game.click_on_start_button?(e.x, e.y) }
show